...
export abstract class DbBase {
    ...
    static init() {
        DbBase.isTest = config.has('test') === true? config.get<boolean>('test') : false;
        ...
        DbBase.connection = config.get('connection');
        DbBase.dbConfig = config.get('db');
    }
    ...
    constructor(dbName: string) {
        let myConfig = DbBase.dbConfig[dbName];
        ...
        if (DbBase.isTest === true) this.databaseName += '$test';
        let conn = myConfig.connection;
        let connection: any;
        if (conn === undefined) {
            connection = DbBase.connection;
        }
        else {
